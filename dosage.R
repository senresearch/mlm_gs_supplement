...
pvalsDos = lapply(1:6, function(i){
  read.csv(paste("./processed/p", i, "_pvalsDos.csv", sep=""))
})
pvals = lapply(1:6, function(i){
  read.csv(paste("./processed/p", i, "_pvals.csv", sep=""))
})
pvalsCond = lapply(1:6, function(i){
  read.csv(paste("./processed/p", i, "_pvalsCond.csv", sep=""))
})
SPvals = lapply(1:6, function(i){
  read.csv(paste("./processed/p", i, "_SPvals.csv", sep=""))
})
SPvalsCond = lapply(1:6, function(i){
  read.csv(paste("./processed/p", i, "_SPvalsCond.csv", sep=""))
})
...
get_prop = function(adjPvals, FDRs) {
  out = numeric(length(FDRs))
  for (i in 1:length(FDRs)) {
    out[i] = mean(adjPvals <= FDRs[i])
  }
  return(out)
}

# Array storing the proportion of hits for each scenario
hits = array(dim=c(6, length(FDRs), 5))
for (i in 1:6) {
  hits[i,,1] = get_prop(adjPvalsDos[[i]], FDRs) # Dosage MLM 
  hits[i,,2] = get_prop(adjPvals[[i]], FDRs) # Categorical cond-conc MLM
  hits[i,,3] = get_prop(adjSPvals[[i]], FDRs) # Categorical cond-conc Collins
  hits[i,,4] = get_prop(adjPvalsCond[[i]], FDRs) # Categorical conditions MLM
  hits[i,,5] = get_prop(adjSPvalsCond[[i]], FDRs) # Categorical conditions Collins
}
...
sapply(1:6, function(i) {
  plot(FDRs, hits[i,,1], type="l", col=mycols[3], lty=mylines[3], # Dosage MLM 
       xlab="Adjusted p-value Cutoffs", ylab="Prop. of Significant Interactions")
  lines(FDRs, hits[i,,2], col=mycols[1], lty=mylines[1]) # Categorical cond-conc MLM
  lines(FDRs, hits[i,,3], col=mycols[2], lty=mylines[2]) # Categorical cond-conc Collins
  legend(0.475, 0.275,
